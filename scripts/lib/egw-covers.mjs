...
export async function generateCovers({ root, concurrency = 4 } = {}) {
  const catalog = JSON.parse(await readFile(path.join(root, "data", "catalog.json"), "utf8")).books; const statePath = path.join(root, ".egw-state", "covers.json"); const reportPath = path.join(root, "build", "egw-covers-report.json"); let state; try { state = JSON.parse(await readFile(statePath, "utf8")); } catch { state = { schema_version: 1, completed: [] }; }
