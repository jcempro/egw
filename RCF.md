...
- `RCF-IF-DATA-001` Cada Livro DEVE possuir exatamente um diretório canônico sob `/d/` e um único `metadata.json`; PDF, EPUB, capa, pacote compacto e demais assets exclusivos do Livro DEVEM coexistir nesse mesmo diretório físico e URL, sem árvore paralela ou duplicação por tipo de asset.
...
- `RCF-IF-DATA-027` Título editorial DEVE permanecer íntegro em `metadata.json`; a decomposição em segmentos existe somente para roteamento e NÃO DEVE alterar, abreviar nem fundir idioma, categoria, volume, edição ou tag ao título exibido.
...
- `RCF-IF-PKG-001` Cada Livro com artefato preservado DEVE publicar, no próprio diretório canônico, os PDFs e EPUBs aceitos e exatamente um pacote compacto canônico. O formato do pacote DEVE ser declarado na configuração versionada; `.7z` com LZMA2 e nível máximo disponível DEVERIA ser o padrão, e `.zip` PODE substituí-lo por requisito de compatibilidade explícito.
- `RCF-IF-PKG-002` PDF, EPUB, capa, pacote e `metadata.json` DEVEM usar nomes portáteis, determinísticos e únicos dentro do Livro. Pacote NÃO DEVE conter caminho absoluto, temporário, cache, credencial ou conteúdo de outro Livro; cada PDF ou EPUB publicado solto DEVE corresponder byte a byte ao asset registrado no metadado.
...
- `RCF-IF-DATA-016` `assets` DEVE ser lista não vazia de objetos com `id`, `format`, `url`, `size`, `source_hashes` e `origin_url`; capa, pacote e cada PDF ou EPUB aceito DEVEM ocupar entradas próprias. `source_hashes` DEVE conter SHA-1, SHA-256 e SHA-512 dos bytes servidos pelo `url`, não do conteúdo descompactado ou de outro formato relacionado.
...
- `RCF-IF-DATA-019` `sources` DEVE ser lista ordenada de objetos com `id`, `title`, `url`, `type`, `format`, `provider`, `asset_id` e `hashes`; `id` DEVE ser único no Livro, `url` DEVE conservar integralmente o endereço HTTP(S), `format` DEVE identificar o invólucro realmente baixado e `hashes` DEVE ser `null` ou conter SHA-1, SHA-256 e SHA-512 de seus bytes. `asset_id` DEVE referenciar o asset local byte a byte equivalente quando houver.
...
- `RCF-IF-UX-008` A região de identidade do livro DEVE conter capa quando disponível, título, autor principal ou contribuidores, resumo de finalidade, matriz Hash Global copiável por PDF/EPUB e fatos editoriais essenciais.
...
- `RCF-IF-UX-012` Cada fonte DEVE apresentar, nesta ordem lógica, posição, título, URL integral, tipo, Estado público, matriz Hash da Fonte e último resultado de validação.
...
- `RCF-IF-BUILD-006` O importador do Acervo de entrada DEVE descobrir grupos de forma determinística, validar manifestos e hashes de entrada quando existirem, derivar o diretório `/d/` conforme `RCF-IF-ROUTE-002`, copiar para ele cada artefato aceito, gerar seu `metadata.json` schema 5 e atualizar somente índices mínimos afetados; falha em um grupo NÃO DEVE publicar saída parcial desse grupo.
...
- `RCF-IF-BUILD-010` Insumos editoriais internos sob `src/` DEVEM permanecer separados de `dist/`; o build DEVE regenerar a árvore pública `/d/`, capas PNG, PDFs e EPUBs aceitos, pacote compacto, índices fragmentados e manifestos a partir de insumos validados, sem copiar temporário ou artefato não registrado.
...
- `RCF-IF-ACC-011` Validação da árvore estática DEVE comprovar raiz `/d/`, idioma e categoria isolados, decomposição determinística do título, um diretório por Livro e coexistência de `metadata.json`, `cover.png`, PDFs, EPUBs e pacote correlatos; Hash Global apenas de PDF/EPUB, Hash da Fonte de cada asset, pacote, densidade, manifesto e índices fragmentados DEVEM ser verificados.
